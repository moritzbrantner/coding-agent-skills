...
extensions:
  agent.procedure:
    ...
    termination:
      ...
      evidenceRequired: ["Relevant diagnosis, implementation or review history, CI/verification attempts, exact candidate evidence, traces or profiles when applicable, correlated `agent-run/v1` Performance Evidence when available, and available task/handoff context are inspected; direct evidence is distinguished from inference."]
    ...
    artifacts:
      consumes: ["engineering-incident-evidence", "diagnosis-envelope", "review-findings", "verification-evidence", "agent-run/v1"]
...
Prefer exact commits, PR heads, check results, profiles, traces, diagnosis envelopes, review findings, task packets, handoff receipts, and canonical evidence when available. Do not infer a process failure merely because the repair was difficult.

When the `performance-evidence` `agent-run/v1` profile is available, correlate records by the producer-owned run/attempt identity and exact source/candidate provenance rather than by timestamps or filenames. Useful evidence can include invocation/retry counts, execution time, deterministic time-to-green, CI/tool/wait spans, token categories, escalation stage, and whether a candidate was produced. Treat missing telemetry as unknown rather than zero. The generic `performance-evidence` artifact produced by runtime diagnosis is a different semantic input and must not be substituted for `agent-run/v1`.
...
Use correlated `agent-run/v1` evidence to test concrete hypotheses such as:
...
- relevant correlated `agent-run/v1` cost evidence when it materially explains the delay or prevention opportunity;
